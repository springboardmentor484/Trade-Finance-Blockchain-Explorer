...
export default function DocumentDetail() {
  ...
  return (
    <div className="p-6">
      ...
      <p className="mb-3">
        Status: <span className="font-medium">{doc.document.status}</span>
      </p>

      {doc.document.is_compromised && (
        <div className="mb-4 p-3 bg-red-100 text-red-800 rounded">
          ⚠️ Warning: This document failed integrity verification.
        </div>
      )}
      ...
      <div className="space-x-2 mb-4">
        {role === "seller" && docType === "PO" && status === "CREATED" && (
          <button
            className="mt-2 px-3 py-1 bg-orange-500 text-white rounded hover:bg-orange-700"
        ...
        {role === "auditor" && status !== "VERIFY" && (
          <button
            className="mt-2 px-3 py-1 bg-purple-500 text-white rounded hover:bg-purple-700"
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="inline-block mt-2 px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-700"
      >
        Download File
      </a>
